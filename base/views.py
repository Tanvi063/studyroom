...
def room(request, pk):
    # Fetch the room based on primary key (pk)
    room = Room.objects.get(id=pk)
    
    # Get all messages related to the room, sorted by creation date (descending)
    ...
    if request.method == 'POST':
        # Handle file upload and message creation
        file = request.FILES.get('file')  # Fetch the uploaded file
        body = request.POST.get('body')  # Get the message content
        ...
        message = Message.objects.create(
            ...
            room=room,
            body=body,
            file=file,  # Assign the uploaded file
        )
        
        # Add the current user to the room's participants (Many-to-Many field)
        room.participants.add(request.user)
        
        # Redirect back to the same room
    ...
    # Render the room template with the context
